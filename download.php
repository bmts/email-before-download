...
if ($ebd_link->selected_id != NULL && $ebd_link->selected_id != 0) {
    ...
    if (!$is_new_dm) {
        ...
        $file = do_shortcode('[download_data id="' . $ebd_link->selected_id . '" data="download_link"]');

    $wpdb->update($table_link, array(
        "is_downloaded" => 1
    ), array(
        "uid" => esc_sql($dId)
    ));
...
if ($is_masked && function_exists('curl_init')) {
    ...
    if ($is_force_download)
        ...
        header("Content-Disposition: filename=\"" . $base_file_name . "\"");
